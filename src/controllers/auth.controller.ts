...
export const syncUser = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (!user) {
      ...
      const currentRole = sessionClaims?.publicMetadata?.role as string || 'user';
...
/**
 * Sync Admin User
 * Dedicated endpoint for Admin Portal to ensure user is created as Admin
 * POST /api/auth/sync-admin
 */
export const syncAdmin = async (req: Request, res: Response): Promise<void> => {
  try {
    const { email, name } = req.body;
    ...
    console.log(`👮‍♂️ Syncing Admin User: ${userId}`);

    // 1. Try to find the user
    let user = await User.findOne({ clerkId: userId });

    if (!user) {
      // 2. IF USER MISSING: Create as ADMIN
      console.log(`✨ Creating NEW Admin user: ${userId}`);

      // Auto-assign role in Clerk Metadata first so next time it's persistent
      ...
      user = await User.create({
        ...
        email,
        name,
        role: 'admin', // FORCED ROLE for Admin Portal
        balance: 0,
        gamesPlayed: 0,
        gamesWon: 0,
        totalWinnings: 0
      ...
      // 3. IF USER EXISTS: Strict role check — only allow admin/super_admin/moderator
      const allowedAdminRoles = ['admin', 'super_admin', 'moderator'];
      if (!allowedAdminRoles.includes(user.role)) {
        console.warn(`🚫 Blocked access for user ${userId} — role is '${user.role}', not an admin role.`);
        res.status(403).json({
          success: false,
      ...
      // Update details for valid admin users
      if (email && user.email !== email) user.email = email;
      if (name && user.name !== name) user.name = name;
      await user.save();
    }
    ...
    res.status(500).json({ success: false, message: "Failed to sync admin" });
  }
};
